...
def generator_and_load_files(
    ...
    plant_map = (
        ...
        .drop_duplicates(subset="Resource")
    )
    # pick up Resource → unit_id_pg map (one per cluster)
    unit_map = (
        gens_by_model_year[["Resource", "unit_id_pg"]]
        .drop_duplicates(subset="Resource")
    )
    info = (
        info
        .merge(plant_map, left_on="GENERATION_PROJECT", right_on="Resource", how="left")
        .merge(unit_map,  left_on="GENERATION_PROJECT", right_on="Resource", how="left", suffixes=("", "_unit"))
    )
    info['plant_id'] = (
        info['plant_gen_id']
        .str.extract(r'^(\d+)')[0]    # pulls the digits before "_"
        .astype('Int64')              # nullable integer, so missing stay <NA>
        )
    info.drop(columns=["Resource","Resource_unit"], errors="ignore", inplace=True)
    info.to_csv(out_folder / "gen_info2.csv", index=False)
...
def gen_candidate_file(gens_by_build_year, info, out_folder):
    """
    Output columns
    ...
    candidates = gens_by_build_year.loc[gens_by_build_year["existing"], gbp_cols.keys()]
    candidates = candidates.rename(columns=gbp_cols)
    candidates = info[~info['GENERATION_PROJECT'].isin(candidates['GENERATION_PROJECT'])]
    ...
    if first_year_settings.get("emission_policies_fn"):
        ...
        for model_year, scen_settings in scen_settings_dict.items():
            try:
               ...
               case_id = scen_settings.get("case_id", "UNKNOWN")
               model_year = scen_settings.get("model_year", "UNKNOWN")
               print(f"⚠️⚠️⚠️ Missing policy data for case_id={case_id}, year={model_year} ({e})⚠️⚠️⚠️")
               co2_cap = None
                # The PG 'policies' table doesn't have this (case_id, year); skip it.
        ...
        for model_year, scen_settings in scen_settings_dict.items():
            try:
                ...
                case_id = scen_settings.get("case_id", "UNKNOWN")
                model_year = scen_settings.get("model_year", "UNKNOWN")
                print(f"⚠️  Missing ESR policy data for case_id={case_id}, year={model_year} ({e})⚠️")
        ...
        for model_year, scen_settings in scen_settings_dict.items():
            try:
                ...
                case_id = scen_settings.get("case_id", "UNKNOWN")
                model_year = scen_settings.get("model_year", "UNKNOWN")
                print(f"⚠️⚠️  Missing MinCap data for case_id={case_id}, year={model_year} ({e})⚠️⚠️")
                mcr = None
            if mcr is not None:
